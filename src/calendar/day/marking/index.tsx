...
export default class Marking extends Component {
  ...
  getItems(items) {
    ...
    if (items && Array.isArray(items) && items.length > 0) {
      ...
      const validItems = _.filter(items, function (o) {
        return o.color;
      });
  ...
  renderDot(index, item) {
    const {selected, dotColor, dots, marked} = this.props;
    ...
    return <Dot {...dotProps} marked={marked} customDot={dots} key={key} color={color} />;
  }
  render() {
